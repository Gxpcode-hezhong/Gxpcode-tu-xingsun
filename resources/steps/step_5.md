...
```bash
python "${SKILL_DIR}/scripts/step5_notify.py" .gxpcode
```
...
- `.gxpcode/s4/`：S4 分析结果（含 summary / tags / applicability / reason / needs_manual_review）
...
- `.gxpcode/s5_report_{date}.md`：Markdown 报告
- `.gxpcode/s5_report_{date}.pdf`：PDF 报告（微软雅黑，A4）
- `{SKILL_DIR}/.gxpcode/history.json`：更新历史记录（按 source 分组，title+url 去重）
