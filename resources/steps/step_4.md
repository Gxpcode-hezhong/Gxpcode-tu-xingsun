...
```
检查 .gxpcode/s3/.done 是否存在 → 不存在则等待 S3
```

## 输入

- `.gxpcode/s3/`：S3 每条含 `title` + `detail_text` + `attachment`
...
```bash
python "${SKILL_DIR}/scripts/step4_merge.py" .gxpcode analysis.json
```
...
- `.gxpcode/s4/s4_{源名}.json`：每条追加 5 个分析字段
- `.gxpcode/s4/.done`：标记完成
